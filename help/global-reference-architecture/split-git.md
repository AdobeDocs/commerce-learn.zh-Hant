...
* 您可以透過此模型安裝任何Composer套件，包括模組、主題、語言套件、composer-plugin、composer-metapackage、magento2-component和修補程式
...
* 在大多數情況下，大量套件模式是較好的替代方案，因為它同樣簡單，但更傳統
...
這兩個存放庫會儲存協力廠商套件和GRA套件。 某些程式碼是每個Adobe Commerce例項所獨有的。 建立一個位置，將這些本機套件儲存在gra-split-brand-x存放庫中。
...
Adobe Commerce是撰寫器應用程式。 首選的安裝方式一律透過Composer存放庫。 只有當模組供應商不透過Composer存放庫提供安裝時，您才能將第三方模組儲存在第三方存放庫中。 自訂程式碼的偏好位置位於GRA存放庫中。 當一個特定執行個體使用模組時，它就會變成本機程式碼。
...
現在可以直接從`packages/3rdparty`、`packages/gra`和`packages/local`目錄認可變更。 Git會將變更提交至與目錄符號連結的Git存放庫。 在`packages/3rdparty`、`packages/gra`或`packages/local`目錄中發出Git認可命令很重要。 請勿在專案根目錄執行Git認可。
...
如果您看到上述輸出，則可以安全地將其提交至品牌存放庫。 若要確認您認可到正確的遠端，請執行`git remote -v`。
...
若要將程式碼傳送至Adobe Commerce執行個體，請將GRA和第三方存放庫合併至gra-split-brand-x存放庫。 執行`composer require`、`bin/magento module:enable`並認可結果。
