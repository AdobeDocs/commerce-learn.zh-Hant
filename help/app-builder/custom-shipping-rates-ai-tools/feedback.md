---
title: 使用AI工具的自訂運費|提供意見回饋
description: 瞭解如何使用AI工具建立自訂運費。 透過此對初學者友善的教學課程，瞭解意見反應流程並簡化工作流程。
feature: Tools and External Services
topic: Artificial Intelligence
role: Developer
level: Beginner
doc-type: Tutorial
duration: 76
last-substantial-update: 2026-03-10T00:00:00Z
jira: KT-20518
exl-id: 7d1487be-2e3b-4067-8382-297a040a820b
source-git-commit: f6eafb3a9d604d4111fbf44384ac7944815c8dd8
workflow-type: tm+mt
source-wordcount: '115'
ht-degree: 0%

---

# 使用AI工具建立自訂運費 — 提供意見回饋

瞭解如何使用AI工具建立自訂運費。 本教學課程會引導您完成意見反應流程，有助於簡化工作流程並提高效率。 瞭解如何有效輸入意見並壓縮交談，確保使用MCP工具獲得順暢的體驗。

## 這部影片是給誰看的？

* 技術架構師
* 資深開發人員或工程師
* 產品經理

## 視訊內容

* 瞭解MCP意見回饋工具的運作方式
* 瞭解如何從IDE為Adobe提供意見回饋

>[!VIDEO](https://video.tv.adobe.com/v/3481317?captions=chi_hant&learn=on)

{{$include /help/_includes/ai-shipping-rates-related-links.md}}
