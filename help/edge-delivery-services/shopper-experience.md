---
title: 使用Edge Delivery Services的購物者體驗
description: 瞭解購物者在Adobe Commerce Edge Delivery Services店面的體驗，包括購物車、結帳、折扣、帳戶工作、訂單和退貨。
feature: Best Practices, Compliance, Extensibility, Storefront, Tools and External Services
topic: Architecture, Commerce, Headless
role: User
level: Beginner
doc-type: Technical Video
duration: 588
last-substantial-update: 2024-02-21T00:00:00.000Z
jira: KT-17259
exl-id: 27a2aaa6-7748-4ec2-919f-9e5dc07e94e7
TQID: https://experienceleague.adobe.com/3AYupbUJJncbDDt6wnSXU0MO2Em-6gOeCbzwgxMzpBQ
product_v2:
  - id: eadea719-cf89-469b-a6fd-a236a7138047
feature_v2:
  - id: c1256247-af4b-46d8-9dca-0c654ecfa157
  - id: d1e21356-0064-4f48-9089-16e3f0dbd2a6
  - id: dac87252-6066-4d6e-a9d2-f6d84c323de7
  - id: e8818fe6-9c8b-4bc0-9ef8-377a10b7bc75
role_v2:
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: d394484608d6ee530932387938e04030ed3b590e
workflow-type: tm+mt
source-wordcount: 174
ht-degree: 0%

---

# 購物者體驗

瞭解Edge Delivery Services上的Commerce店面如何支援常見的購物者工作：新增商品、套用折扣、訪客結帳、帳戶更新、取消訂單、重新排序和退貨。 影片也涵蓋Order Management、客戶如何套用折扣，以及何時可以棧疊多個優惠券代碼。

## 您能學到的內容

您將瞭解如何在Edge Delivery Services上瀏覽Adobe Commerce店面，包括購物車管理、折扣、訪客結帳、帳戶更新、訂單動作和結帳。

## 這部影片是給誰看的？

* 電子商務企業所有者
* 網站管理員
* 數位行銷人員
* 客戶服務代表
* IT專業人員
* 線上商店管理員
* UX/UI設計工具
* 銷售經理
* 行銷專家
* 產品經理

## 視訊內容

* 在結帳期間（包括提供棧疊時）套用購物車價格規則和優惠券代碼。
* 使用購物車與訂單：新增料號、以客人身份結帳、取消訂單、重新排序以及處理退貨。
* 更新帳戶資訊、合併來賓和客戶購物車，以及維護結帳一致性。

>[!VIDEO](https://video.tv.adobe.com/v/3446773?captions=chi_hant&learn=on)
