---
title: 利用可組合性建置靈活的Commerce體驗
description: 瞭解Adobe Commerce的可撰寫架構如何提供模組化彈性、Headless支援，以及API優先的整合。
feature: Extensibility, Admin Workspace, Integration
topic: Architecture, Development
role: Developer, User
level: Beginner
doc-type: Technical Video
duration: 181
last-substantial-update: 2026-01-12T00:00:00Z
jira: KT-20092
exl-id: fdbb45ca-b1dc-4015-bd4d-bbe90c442dbb
source-git-commit: b859664f02cf6eac99a551e5f58dff34ca55e37a
workflow-type: tm+mt
source-wordcount: '98'
ht-degree: 0%

---

# 利用可組合性建置彈性的商務體驗

瞭解Adobe Commerce as a Cloud Service如何提供可組合的架構，讓企業獲得靈活度與擴充能力。 瞭解模組化服務、API優先整合和管理UI擴充性如何在不影響效能的情況下實現量身打造的商務體驗。

## 這部影片是給誰看的？

* 解決方案架構師
* 前端開發人員
* Ecommerce Manager

## 視訊內容

* 瞭解可組合性如何支援模組化架構、Headless商務和API優先整合。
* 瞭解管理員UI擴充性和開發人員工具如何簡化自訂和工作流程增強功能。

>[!VIDEO](https://video.tv.adobe.com/v/3478798?learn=on)
